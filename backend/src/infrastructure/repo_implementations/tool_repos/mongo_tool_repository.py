...
class MongoToolRepository(IToolRepository):
    ...
    async def search(
        ...
        try:
            ...
            if query != "":
                match_stage["$match"].update({"$text": {"$search": query}})
            if category:
            ...
            pipeline = [
                match_stage,
                {"$addFields": {"score": {"$meta": "textScore"}}} if "$text" in match_stage["$match"] else {},
                {"$sort": {"score": -1, "_id": 1}} if "$text" in match_stage["$match"] else {"$sort": {"_id": 1}},
                {"$skip": skip},
                {"$limit": page_size},
                {"$project": {
                    "_id": 1,
